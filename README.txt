...
> 	in Voucher.cpp in methods: 	void Voucher::ChangeTime(tm date);
/					float Voucher::PriceRate();
/	use {time_t now = time(0); tm * currentTime = new tm(); currentTime = gmtime(&now);}
/	instead of {time_t now = time(0); tm currentTime = tm(); gmtime_s(&currentTime, &now);};

>	also in this methods amend '.' to '->';